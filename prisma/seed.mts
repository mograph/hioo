...
async function main() {
  ...
  const outfits = await Promise.all([
    prisma.outfit.create({
      data: {
        userId: user.id, name: 'Effortless Monday', occasion: 'business', date: new Date('2026-03-23'),
        items: { create: [{ clothingItemId: items[0].id }, { clothingItemId: items[9].id }, { clothingItemId: items[16].id }, { clothingItemId: items[21].id }] },
      },
    }),
    prisma.outfit.create({
      data: {
        userId: user.id, name: 'Weekend Coffee Run', occasion: 'casual', date: new Date('2026-03-22'),
        items: { create: [{ clothingItemId: items[1].id }, { clothingItemId: items[8].id }, { clothingItemId: items[13].id }, { clothingItemId: items[24].id }] },
      },
    }),
    prisma.outfit.create({
      data: {
        userId: user.id, name: 'Spring Brunch', occasion: 'casual', date: new Date('2026-03-21'),
        items: { create: [{ clothingItemId: items[4].id }, { clothingItemId: items[11].id }, { clothingItemId: items[15].id }, { clothingItemId: items[23].id }] },
      },
    }),
    prisma.outfit.create({
      data: {
        userId: user.id, name: 'Dinner Date', occasion: 'formal', date: new Date('2026-03-20'),
        items: { create: [{ clothingItemId: items[1].id }, { clothingItemId: items[9].id }, { clothingItemId: items[14].id }, { clothingItemId: items[17].id }, { clothingItemId: items[21].id }] },
      },
    }),
    prisma.outfit.create({
      data: {
        userId: user.id, name: 'Rainy Day Layers', occasion: 'casual', date: new Date('2026-03-19'),
        items: { create: [{ clothingItemId: items[3].id }, { clothingItemId: items[10].id }, { clothingItemId: items[14].id }, { clothingItemId: items[19].id }] },
      },
    }),
    prisma.outfit.create({
      data: {
        userId: user.id, name: 'French Girl Friday', occasion: 'casual', date: new Date('2026-03-18'),
        items: { create: [{ clothingItemId: items[2].id }, { clothingItemId: items[8].id }, { clothingItemId: items[13].id }, { clothingItemId: items[22].id }] },
      },
    }),
    prisma.outfit.create({
      data: {
        userId: user.id, name: 'Cozy WFH', occasion: 'casual', date: new Date('2026-03-17'),
        items: { create: [{ clothingItemId: items[5].id }, { clothingItemId: items[8].id }, { clothingItemId: items[13].id }] },
      },
    }),
    prisma.outfit.create({
      data: {
        userId: user.id, name: 'Summer Office', occasion: 'business', date: new Date('2026-03-16'),
        items: { create: [{ clothingItemId: items[7].id }, { clothingItemId: items[12].id }, { clothingItemId: items[16].id }, { clothingItemId: items[24].id }] },
      },
    }),
  ])
  ...
  const wearLogData = [
    ...
    { outfitId: outfits[3].id, date: new Date('2026-03-12'), notes: 'Wore this again' },
  ...
  for (const log of wearLogData) {
    await prisma.wearLog.create({
      data: { userId: user.id, outfitId: log.outfitId, date: log.date, notes: log.notes },
    })
  }
